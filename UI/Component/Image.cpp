...
namespace Engine {
	...
	// 		Size.x, Size.y, 0);
	// }
	void Image::Draw(float scale, float cx, float cy, float sx, float sy) const {
		al_draw_scaled_bitmap(bmp.get(), 0, 0, GetBitmapWidth(), GetBitmapHeight(),
			(Position.x - Anchor.x * GetBitmapWidth() 	- sx)*scale + cx,
			(Position.y - Anchor.y * GetBitmapHeight()	- sy)*scale + cy,
			Size.x * scale, Size.y * scale, 0);
	}
